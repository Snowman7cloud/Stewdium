...
export default function Stewdium(){
  ...
  {page==="addRecipe"&&newRecipe&&<div style={{maxWidth:640,margin:"0 auto"}}><button className="back-btn" onClick={()=>{setNewRecipe(null);setPage("board");}}>← Back</button>
    ...
    <div className="card-static" style={{padding:28}}>
      ...
      <div className="form-group"><label className="form-label">Diet Tags <span style={{fontWeight:400,color:"var(--gray-400)"}}>(auto-suggested when you add ingredients)</span></label><div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{DIET_TAGS.map(t=><button key={t.id} className={`diet-tag-chip ${newRecipe.dietTags?.includes(t.id)?"active":""}`} onClick={()=>setNewRecipe({...newRecipe,dietTags:newRecipe.dietTags?.includes(t.id)?newRecipe.dietTags.filter(x=>x!==t.id):[...(newRecipe.dietTags||[]),t.id]})}>{t.label}</button>)}</div></div>
      ...
      <div className="form-group"><label className="form-label">Ingredients</label>
        ...
        {ingPasteMode?<><textarea className="form-input form-textarea" style={{minHeight:160,fontFamily:"monospace",fontSize:13}} placeholder={"Paste ingredients, one per line:\n\n2 cups all-purpose flour\n1/2 tsp salt\n3 cloves garlic, minced"} value={ingPasteText} onChange={e=>setIngPasteText(e.target.value)}/><button className="btn btn-primary btn-sm" style={{marginTop:8}} onClick={()=>{const parsed=parseIngredientBlock(ingPasteText);if(!parsed.length){alert("Could not parse. Put each ingredient on its own line.");return;}const existing=newRecipe.ingredients.filter(i=>i.name);const autoTags=suggestDietTags(parsed.map(p=>({...p,amount:parseFrac(p.amount)})));setNewRecipe({...newRecipe,ingredients:[...existing,...parsed],dietTags:[...new Set([...(newRecipe.dietTags||[]),...autoTags])]});setIngPasteMode(false);setIngPasteText("");}}>Parse & Add ({parseIngredientBlock(ingPasteText).length} found)</button></>
