...
import { parseIngredientBlock } from '../lib/parse-ingredients';
...
const SORT_OPTS=[{id:"newest",label:"Newest"},{id:"popular",label:"Most Popular"}];
...
function NutritionPanel({nutrition}){
  ...
  const items=[{label:"Calories",val:ps.calories,unit:"kcal",color:"var(--pink-400)"},{label:"Protein",val:ps.protein,unit:"g",color:"var(--sage-500)"},{label:"Carbs",val:ps.carbs,unit:"g",color:"var(--blue-400)"},{label:"Fat",val:ps.fat,unit:"g",color:"var(--pink-300)"}];
  return(<div className="nutrition-panel"><div className="section-title" style={{borderColor:"var(--sage-200)"}}>🥗 Nutrition Per Serving</div><div style={{display:"flex",gap:16,flexWrap:"wrap"}}>{items.map(i=><div key={i.label} style={{flex:1,minWidth:80,textAlign:"center",padding:12,background:"var(--gray-50)",borderRadius:"var(--radius-sm)"}}><div style={{fontSize:22,fontWeight:800,color:i.color}}>{i.val}</div><div style={{fontSize:11,color:"var(--gray-400)",fontWeight:700,textTransform:"uppercase"}}>{i.unit}</div><div style={{fontSize:12,fontWeight:700,color:"var(--gray-600)",marginTop:2}}>{i.label}</div></div>)}</div>{nutrition.matched<nutrition.totalIngredients&&<div style={{fontSize:11,color:"var(--gray-400)",marginTop:8,textAlign:"center"}}>Based on {nutrition.coverage} matched ingredients. Values are estimates.</div>}</div>);
}

function AllergenBadges({allergens,userAllergies=[]}){
  if(!allergens?.length)return null;
  const dangerous=allergens.filter(a=>userAllergies.includes(a));
  const safe=allergens.filter(a=>!userAllergies.includes(a));
  return(<div style={{display:"flex",gap:4,flexWrap:"wrap",marginTop:6}}>{dangerous.map(a=>{const al=ALLERGEN_LIST.find(x=>x.id===a);return al?<span key={a} style={{padding:"2px 8px",borderRadius:99,fontSize:11,fontWeight:700,background:"#fef2f2",color:"#ef4444",border:"1px solid #fecaca"}}>{al.icon} {al.label}</span>:null;})}{safe.map(a=>{const al=ALLERGEN_LIST.find(x=>x.id===a);return al?<span key={a} style={{padding:"2px 8px",borderRadius:99,fontSize:11,fontWeight:600,background:"var(--gray-100)",color:"var(--gray-500)"}}>{al.icon} {al.label}</span>:null;})}</div>);
}
...
.nutrition-panel{margin:24px 0;padding:20px;background:var(--white);border-radius:var(--radius);box-shadow:var(--shadow-sm)}
.like-btn{display:inline-flex;align-items:center;gap:4px;padding:6px 14px;border-radius:99px;border:2px solid var(--pink-200);background:var(--white);font-size:13px;font-weight:700;cursor:pointer;font-family:var(--font-body);color:var(--pink-400);transition:all .15s}.like-btn:hover{background:var(--pink-50);border-color:var(--pink-300)}.like-btn.liked{background:var(--pink-100);border-color:var(--pink-300)}
...
.diet-tag-grid{display:flex;gap:6px;flex-wrap:wrap;margin-top:6px}.diet-tag-chip{padding:5px 12px;border-radius:99px;border:2px solid var(--gray-200);background:var(--white);font-size:12px;font-weight:600;cursor:pointer;font-family:var(--font-body);color:var(--gray-500);transition:all .15s}.diet-tag-chip.active{background:var(--sage-100);border-color:var(--sage-300);color:var(--sage-600)}
.allergy-grid{display:flex;gap:6px;flex-wrap:wrap;margin-top:6px}.allergy-chip{padding:5px 12px;border-radius:99px;border:2px solid var(--gray-200);background:var(--white);font-size:12px;font-weight:600;cursor:pointer;font-family:var(--font-body);color:var(--gray-500)}.allergy-chip.active{background:#fef2f2;border-color:#fecaca;color:#dc2626}
@media print{.nav,.footer,.scale-bar,.recipe-actions,.cooked-grid,.back-btn,.btn,.like-btn,.allergen-warn,.nutrition-panel{display:none!important}.recipe-hero{height:200px}.main{padding:0}.recipe-detail{max-width:100%}}
...
export default function Stewdium(){
  ...
  const[viewing,setViewing]=useState(null);const[cookedPhotos,setCookedPhotos]=useState([]);
  ...
  const[csvMapping,setCsvMapping]=useState(null); // {headers, rows, mapping}

  // Auth
  useEffect(()=>{const{data:{subscription}}=db.onAuthChange(async(ev,sess)=>{if(sess?.user){setUser(sess.user);const{data:p}=await db.getProfile(sess.user.id);setProfile(p);const{data:si}=await db.getSavedRecipeIds(sess.user.id);setSavedIds(si);const{data:li}=await db.getUserLikes(sess.user.id);setLikedIds(li);}else{setUser(null);setProfile(null);setSavedIds([]);setLikedIds([]);}});return()=>subscription.unsubscribe();},[]);
  ...
  useEffect(()=>{if(viewing)db.getCookedPhotos(viewing.id).then(({data})=>setCookedPhotos(data));},[viewing]);
  ...
  const handleSignOut=async()=>{await db.signOut();setPage("home");};
  ...
  const handleAddRecipe=()=>{if(!user){setAuthModal("login");return;}setNewRecipe({title:"",description:"",category:"Dinner",prepTime:"",cookTime:"",servings:4,ingredients:[{amount:"",unit:"",name:""}],steps:[""],isPublic:true,imageFile:null,emoji:"🍽️",dietTags:[]});setPage("addRecipe");};
  ...
  const ings=newRecipe.ingredients.filter(i=>i.name).map(i=>({...i,amount:parseFloat(i.amount)||1}));const steps=newRecipe.steps.filter(s=>s.trim());
  const nutrition=calculateNutrition(ings,parseInt(newRecipe.servings)||4);const detectedAllergens=detectAllergens(ings);
  const{error}=await db.createRecipe({user_id:user.id,title:newRecipe.title,description:newRecipe.description,category:newRecipe.category,prep_time:newRecipe.prepTime,cook_time:newRecipe.cookTime,servings:parseInt(newRecipe.servings)||4,is_public:newRecipe.isPublic,image_url,emoji:newRecipe.emoji,ingredients:ings,steps,allergen_tags:detectedAllergens,nutrition});
  setSaving(false);if(!error){setNewRecipe(null);setPage("board");loadRecipes();}};
  ...
  const importCSV=async e=>{const file=e.target.files?.[0];if(!file||!user)return;const text=await file.text();const lines=[];let cur="",inQ=false;for(let i=0;i<text.length;i++){const ch=text[i];if(ch==='"'){if(inQ&&text[i+1]==='"'){cur+='"';i++;}else inQ=!inQ;}else if(ch==="\n"&&!inQ){lines.push(cur);cur="";}else cur+=ch;}if(cur.trim())lines.push(cur);
  const pr=line=>{const cols=[];let c="",q=false;for(let i=0;i<line.length;i++){const ch=line[i];if(ch==='"'){if(q&&line[i+1]==='"'){c+='"';i++;}else q=!q;}else if(ch===","&&!q){cols.push(c);c="";}else c+=ch;}cols.push(c);return cols;};
  if(lines.length<2){alert("CSV needs a header row and at least one recipe.");e.target.value="";return;}
  const headers=pr(lines[0]).map(h=>h.trim());const rows=lines.slice(1).filter(l=>l.trim()).map(l=>pr(l));
  const autoMap={};const fields=["title","description","category","prep_time","cook_time","servings","is_public","ingredients","steps"];
  const aliases={title:["title","name","recipe"],description:["description","desc","about"],category:["category","type","meal"],prep_time:["prep time","prep","preparation"],cook_time:["cook time","cook","cooking"],servings:["servings","serves","yield"],is_public:["public","visibility","shared"],ingredients:["ingredients","ingredient"],steps:["steps","instructions","directions","method"]};
  headers.forEach((h,i)=>{const lh=h.toLowerCase();for(const[field,words]of Object.entries(aliases)){if(!autoMap[field]&&words.some(w=>lh.includes(w))){autoMap[field]=i;break;}}});
  setCsvMapping({headers,rows,mapping:autoMap});e.target.value="";setPage("csvMap");};

  const finalizeCsvImport=async()=>{if(!csvMapping||!user)return;const{headers,rows,mapping:m}=csvMapping;const nr=[];
  for(const cols of rows){const title=m.title!==undefined?cols[m.title]?.trim():"";if(!title)continue;
  const ingStr=m.ingredients!==undefined?cols[m.ingredients]||"":"";const ingredients=ingStr.split("|").filter(s=>s.trim()).map(s=>{const parts=s.trim().match(/^([\d./]+)\s*(\S+)\s+(.+)$/);if(parts)return{amount:parseFloat(parts[1])||1,unit:parts[2],name:parts[3]};return{amount:1,unit:"whole",name:s.trim()};});
  const stepStr=m.steps!==undefined?cols[m.steps]||"":"";const steps=stepStr.split("|").filter(s=>s.trim()).map(s=>s.trim());
  const servings=m.servings!==undefined?parseInt(cols[m.servings])||4:4;
  const nutrition=calculateNutrition(ingredients,servings);const allergen_tags=detectAllergens(ingredients);
  nr.push({user_id:user.id,title,description:m.description!==undefined?cols[m.description]?.trim()||"":"",category:m.category!==undefined?cols[m.category]?.trim()||"Dinner":"Dinner",prep_time:m.prep_time!==undefined?cols[m.prep_time]?.trim()||"":"",cook_time:m.cook_time!==undefined?cols[m.cook_time]?.trim()||"":"",servings,is_public:m.is_public!==undefined?cols[m.is_public]?.trim().toLowerCase()!=="no":true,ingredients,steps,emoji:"🍽️",allergen_tags,nutrition});}
  if(nr.length===0)alert("No valid recipes found.");
  else{const{error}=await db.bulkCreateRecipes(nr);if(!error){alert(`Imported ${nr.length} recipe${nr.length>1?"s":""}!`);setCsvMapping(null);setPage("board");loadRecipes();}else alert("Import error: "+error.message);}};
  ...
  const toggleAllergy=async allergyId=>{const current=profile?.allergies||[];const next=current.includes(allergyId)?current.filter(a=>a!==allergyId):[...current,allergyId];setProfile(p=>({...p,allergies:next}));await db.updateProfile(user.id,{allergies:next});};
  ...
  const displayRecipes=useMemo(()=>{if(!allergenFilter||!userAllergies.length)return recipes;return recipes.filter(r=>{const ra=r.allergen_tags||detectAllergens(r.ingredients||[]);return!ra.some(a=>userAllergies.includes(a));});},[recipes,allergenFilter,userAllergies]);

  const recipeHasUserAllergen=(r)=>{if(!userAllergies.length)return false;const ra=r.allergen_tags||detectAllergens(r.ingredients||[]);return ra.some(a=>userAllergies.includes(a));};
  ...
  {page==="home"&&<><div className="hero-section"><div className="hero-title">Welcome to <span>Stewdium</span></div><div className="hero-sub">Discover recipes, build your collection, and plan your meals.</div></div>
    <div className="search-bar">
      <input className="search-input" placeholder="Search recipes or ingredients..." value={search} onChange={e=>setSearch(e.target.value)}/>
      <select className="form-input" style={{width:"auto",borderRadius:99,padding:"10px 14px",fontWeight:600,fontSize:13}} value={sortBy} onChange={e=>setSortBy(e.target.value)}>{SORT_OPTS.map(s=><option key={s.id} value={s.id}>{s.label}</option>)}</select>
      <button className="btn btn-primary" onClick={handleAddRecipe}>+ Add Recipe</button>
    </div>
    <div style={{display:"flex",gap:12,marginBottom:24,flexWrap:"wrap",alignItems:"center"}}>
      <div className="filter-chips">{CATEGORIES.map(c=><button key={c} className={`filter-chip ${catFilter===c?"active":""}`} onClick={()=>setCatFilter(c)}>{c}</button>)}</div>
      {user&&userAllergies.length>0&&<div style={{display:"flex",alignItems:"center",gap:6,marginLeft:"auto"}}><div className={`toggle-track ${allergenFilter?"on":""}`} onClick={()=>setAllergenFilter(!allergenFilter)}><div className="toggle-knob"/></div><span style={{fontSize:13,fontWeight:600,color:"var(--gray-600)"}}>Hide allergens</span></div>}
    </div>
    {loading?<div className="loading"><div className="loading-spinner"/><div>Loading recipes...</div></div>:
      displayRecipes.length===0?<div className="empty-state"><div className="empty-state-icon">🔍</div><div className="empty-state-text">No recipes found</div></div>:
      <div className="recipe-grid">{displayRecipes.map(r=><div key={r.id} className="card recipe-card" onClick={()=>openRecipe(r)}>
        ...
        <div className="recipe-card-body">
          ...
          <div className="recipe-card-author">by {authorName(r)}</div>
          {user&&recipeHasUserAllergen(r)&&<div style={{fontSize:11,color:"#dc2626",fontWeight:700,marginTop:4}}>⚠️ Contains your allergens</div>}
          <div className="recipe-card-meta">
            <span>⏱ {r.cook_time}</span><span>👥 {r.servings}</span>
            <span onClick={e=>{e.stopPropagation();toggleLike(r.id);}} className={`like-btn ${likedIds.includes(r.id)?"liked":""}`}>👍 {r.like_count||0}</span>
          </div>
        </div></div>)}</div>}</>}

  {/* RECIPE DETAIL */}
  {page==="recipe"&&viewing&&(()=>{const r=viewing,scale=getScale(r),es=scaleValue&&scaleMode==="servings"?scaleValue:Math.round(r.servings*scale);
  const nutrition=r.nutrition||calculateNutrition(r.ingredients||[],r.servings);
  const allergens=r.allergen_tags||detectAllergens(r.ingredients||[]);
  return<div className="recipe-detail">
    ...
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:12}}>
      ...
      <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
        {user&&<><button className="btn btn-pink btn-sm" onClick={()=>cookedRef.current?.click()}>📸 I cooked this!</button><input ref={cookedRef} type="file" accept="image/*" onChange={e=>{if(e.target.files?.[0])uploadCookedPhoto(e.target.files[0]);e.target.value="";}} style={{display:"none"}}/></>}
        <button className={`like-btn ${likedIds.includes(r.id)?"liked":""}`} onClick={()=>toggleLike(r.id)}>👍 {r.like_count||0}</button>
        <button className="btn btn-secondary btn-sm" onClick={()=>toggleSave(r.id)}>{savedIds.includes(r.id)?"❤️ Saved":"🤍 Save"}</button>
      </div></div>
    {user&&recipeHasUserAllergen(r)&&<div className="allergen-warn">⚠️ This recipe contains ingredients you're allergic to</div>}
    <AllergenBadges allergens={allergens} userAllergies={userAllergies}/>
    ...
    {r.allergen_tags?.length>0&&<div className="tags" style={{marginBottom:16}}>{(r.allergen_tags||[]).filter(t=>DIET_TAGS.find(d=>d.id===t)).map(t=><span key={t} className="tag">{DIET_TAGS.find(d=>d.id===t)?.label||t}</span>)}</div>}
    ...
    <div className="recipe-actions">
      <button className="btn btn-secondary btn-sm" onClick={printRecipe}>🖨️ Print</button>
      <button className="btn btn-secondary btn-sm" onClick={()=>downloadRecipe(r,scale)}>⬇️ Download</button>
      {!user&&<span style={{fontSize:12,color:"var(--gray-400)",alignSelf:"center"}}>Sign in to print or download</span>}
    </div>
    <div style={{marginTop:32}}><div className="section-title" style={{borderColor:"var(--sage-200)"}}>📸 Community Photos ({cookedPhotos.length})</div>
      {cookedPhotos.length===0?<div style={{textAlign:"center",padding:"32px 16px",color:"var(--gray-400)",background:"var(--gray-50)",borderRadius:"var(--radius-sm)"}}><div style={{fontSize:32,marginBottom:8}}>🍳</div><div style={{fontWeight:600,fontSize:14}}>No photos yet</div></div>
  ...
  {/* BOARD */}
  {page==="board"&&user&&<><div className="page-title">My Recipe Board</div><div className="page-subtitle">Your personal collection</div>
    ...
    <div style={{display:"flex",gap:8,marginBottom:24,flexWrap:"wrap"}}><button className="btn btn-secondary btn-sm" onClick={()=>csvRef.current?.click()}>📥 Import CSV</button><input ref={csvRef} type="file" accept=".csv" onChange={importCSV} style={{display:"none"}}/>{myRecipes.length>0&&<button className="btn btn-secondary btn-sm" onClick={()=>exportCSV(myRecipes)}>📤 Export Mine</button>}{savedRecipes.length>0&&<button className="btn btn-secondary btn-sm" onClick={()=>exportCSV(savedRecipes)}>📤 Export Saved</button>}</div>
  ...
  {page==="addRecipe"&&newRecipe&&<div style={{maxWidth:640,margin:"0 auto"}}><button className="back-btn" onClick={()=>{setNewRecipe(null);setPage("board");}}>← Back</button>
    ...
    <div className="card" style={{padding:28}}>
      ...
      <div className="form-group"><label className="form-label">Diet Tags</label><div className="diet-tag-grid">{DIET_TAGS.map(t=><button key={t.id} className={`diet-tag-chip ${newRecipe.dietTags?.includes(t.id)?"active":""}`} onClick={()=>setNewRecipe({...newRecipe,dietTags:newRecipe.dietTags?.includes(t.id)?newRecipe.dietTags.filter(x=>x!==t.id):[...(newRecipe.dietTags||[]),t.id]})}>{t.label}</button>)}</div></div>
      <div className="form-group"><label className="form-label">Ingredients</label>
        <div style={{display:"flex",gap:8,marginBottom:12}}><button className={`btn btn-sm ${!ingPasteMode?"btn-primary":"btn-secondary"}`} onClick={()=>setIngPasteMode(false)}>Manual Entry</button><button className={`btn btn-sm ${ingPasteMode?"btn-primary":"btn-secondary"}`} onClick={()=>setIngPasteMode(true)}>Paste a List</button></div>
        {ingPasteMode?<>
          <textarea className="form-input form-textarea" style={{minHeight:160,fontFamily:"monospace",fontSize:13}} placeholder={"Paste your ingredient list here, one per line:\n\n2 cups all-purpose flour\n1/2 tsp salt\n3 cloves garlic, minced\n1 lb chicken breast\n2 tbsp olive oil"} value={ingPasteText} onChange={e=>setIngPasteText(e.target.value)}/>
          <button className="btn btn-primary btn-sm" style={{marginTop:8}} onClick={()=>{const parsed=parseIngredientBlock(ingPasteText);if(parsed.length===0){alert("Could not parse any ingredients. Make sure each is on its own line.");return;}setNewRecipe({...newRecipe,ingredients:[...newRecipe.ingredients.filter(i=>i.name),...parsed.map(p=>({amount:String(p.amount),unit:p.unit,name:p.name}))]});setIngPasteMode(false);setIngPasteText("");}}>Parse & Add ({parseIngredientBlock(ingPasteText).length} found)</button>
        </>:<>
        {newRecipe.ingredients.map((ing,i)=><div key={i} style={{display:"flex",gap:8,marginBottom:8,alignItems:"center"}}>
          ...
          <input className="form-input" style={{flex:1}} placeholder="Ingredient" value={ing.name} onChange={e=>{const n=[...newRecipe.ingredients];n[i]={...n[i],name:e.target.value};setNewRecipe({...newRecipe,ingredients:n});}}/>
  ...
  {/* CSV COLUMN MAPPING */}
  {page==="csvMap"&&csvMapping&&<div style={{maxWidth:800,margin:"0 auto"}}>
    <button className="back-btn" onClick={()=>{setCsvMapping(null);setPage("board");}}>← Back</button>
    <div className="page-title">Map CSV Columns</div>
    <div className="page-subtitle">Found {csvMapping.rows.length} recipes. Tell us which columns match which fields.</div>
    <div className="card" style={{padding:28}}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        {[{key:"title",label:"Title *",required:true},{key:"description",label:"Description"},{key:"category",label:"Category"},{key:"prep_time",label:"Prep Time"},{key:"cook_time",label:"Cook Time"},{key:"servings",label:"Servings"},{key:"is_public",label:"Public (Yes/No)"},{key:"ingredients",label:"Ingredients (pipe-separated)"},{key:"steps",label:"Steps (pipe-separated)"}].map(f=>(
          <div key={f.key} className="form-group" style={{marginBottom:12}}>
            <label className="form-label">{f.label}</label>
            <select className="form-input" value={csvMapping.mapping[f.key]!==undefined?csvMapping.mapping[f.key]:""} onChange={e=>{const v=e.target.value;setCsvMapping(prev=>({...prev,mapping:{...prev.mapping,[f.key]:v===""?undefined:parseInt(v)}}));}}>
              <option value="">-- Skip --</option>
              {csvMapping.headers.map((h,i)=><option key={i} value={i}>{h}</option>)}
            </select>
          </div>
        ))}
      </div>
      <div className="section-title" style={{marginTop:16,fontSize:16}}>Preview (first 3 rows)</div>
      <div style={{overflowX:"auto",marginBottom:20}}>
        <table style={{width:"100%",fontSize:12,borderCollapse:"collapse"}}>
          <thead><tr>{csvMapping.headers.map((h,i)=><th key={i} style={{padding:"6px 8px",borderBottom:"2px solid var(--sage-200)",textAlign:"left",fontWeight:700,color:"var(--sage-600)",whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead>
          <tbody>{csvMapping.rows.slice(0,3).map((row,i)=><tr key={i}>{csvMapping.headers.map((_,j)=><td key={j} style={{padding:"6px 8px",borderBottom:"1px solid var(--gray-100)",maxWidth:200,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{row[j]||""}</td>)}</tr>)}</tbody>
        </table>
      </div>
      {csvMapping.mapping.title===undefined&&<div className="error-msg">You must map the Title column to import recipes.</div>}
      <div style={{display:"flex",gap:12}}>
        <button className="btn btn-primary" disabled={csvMapping.mapping.title===undefined} onClick={finalizeCsvImport}>Import {csvMapping.rows.length} Recipes</button>
        <button className="btn btn-secondary" onClick={()=>{setCsvMapping(null);setPage("board");}}>Cancel</button>
      </div>
    </div>
  </div>}

  {/* PLANNER */}
  {page==="planner"&&user&&<><div className="page-title">Meal Planner</div><div className="page-subtitle">Plan your week</div>
    <div className="planner-grid">{DAYS.map(d=><div key={d} className="planner-day"><div className="planner-day-name">{d}</div>
      {MEALS_L.map(m=><div key={m} className="planner-meal"><div className="planner-meal-label">{m}</div>
  ...
  {page==="profile"&&user&&profile&&<><div className="profile-header"><div className="profile-avatar">{profile.avatar_url?<img src={profile.avatar_url} alt=""/>:(profile.name||"U")[0].toUpperCase()}</div><div><div className="profile-name">{profile.name}</div><div className="profile-bio">{profile.bio}</div><div className="profile-stats"><span className="profile-stat">📖 {myRecipes.length} recipes</span><span className="profile-stat">❤️ {savedIds.length} saved</span></div></div></div>
    ...
    <div className="card" style={{padding:28,marginTop:24}}>
      ...
      <div className="allergy-grid">{ALLERGEN_LIST.map(a=><button key={a.id} className={`allergy-chip ${userAllergies.includes(a.id)?"active":""}`} onClick={()=>toggleAllergy(a.id)}>{a.icon} {a.label}</button>)}</div>
      <div style={{display:"flex",alignItems:"center",gap:8,marginTop:16}}><div className={`toggle-track ${profile.allergies_public?"on":""}`} onClick={()=>updateProfileField('allergies_public',!profile.allergies_public)}><div className="toggle-knob"/></div><span style={{fontSize:13,fontWeight:600,color:"var(--gray-600)"}}>{profile.allergies_public?"🌎 Allergies visible to others":"🔒 Allergies are private"}</span></div>
    </div>
    <button className="btn btn-pink" style={{marginTop:24}} onClick={handleSignOut}>Sign Out</button></>}

  </div>

  {/* FOOTER */}
  <footer className="footer"><div className="footer-inner">
    <div className="footer-brand"><div className="footer-brand-name">🍲 <span>stew</span>dium</div><div className="footer-brand-desc">Your home for discovering, sharing, and organizing recipes.</div></div>
