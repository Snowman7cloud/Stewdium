...
export default function Stewdium(){
  ...
  {page==="home"&&<><div className="hero-section"><div className="hero-title">Welcome to <span>Stewdium</span></div><div className="hero-sub">Discover recipes, build your collection, and plan your meals.</div></div>
    ...
    <div style={{display:"flex",gap:12,marginBottom:24,flexWrap:"wrap",alignItems:"center"}}><div className="filter-chips">{CATEGORIES.map(c=><button key={c} className={`filter-chip ${catFilter===c?"active":""}`} onClick={()=>setCatFilter(c)}>{c}</button>)}</div>{user&&uAllergies.length>0&&<div style={{display:"flex",alignItems:"center",gap:6,marginLeft:"auto"}}><div className={`toggle-track ${allergenFilter?"on":""}`} onClick={()=>setAllergenFilter(!allergenFilter)}><div className="toggle-knob"/></div><span style={{fontSize:13,fontWeight:600,color:"var(--gray-600)"}}>Hide allergens</span></div>}</div>
  ...
  {page==="friends"&&user&&<><div className="page-title">Friends</div>
    ...
    {friendResults.length>0&&<div style={{marginBottom:24}}><div style={{fontSize:13,fontWeight:700,color:"var(--gray-500)",marginBottom:8}}>Search Results</div>{friendResults.map(p=><div key={p.id} className="friend-card" onClick={()=>viewProfile(p.id)}><div className="friend-avatar">{p.avatar_url?<img src={p.avatar_url} alt=""/>:(p.name||"?")[0].toUpperCase()}</div><div style={{flex:1}}><div style={{fontWeight:700,fontSize:14}}>{p.name}</div><div style={{fontSize:12,color:"var(--gray-400)"}}>{p.bio}</div></div><button className={`btn btn-sm ${followingIds.includes(p.id)?"btn-secondary":"btn-primary"}`} onClick={e=>{e.stopPropagation();toggleFollow(p.id);}}>{followingIds.includes(p.id)?"Following":"Follow"}</button></div>)}</div>}
    <div style={{fontSize:13,fontWeight:700,color:"var(--gray-500)",marginBottom:8}}>Following ({following.length})</div>
    {following.length===0?<div className="empty-state"><div className="empty-state-icon">👋</div><div className="empty-state-text">Not following anyone yet</div><div style={{fontSize:13,marginTop:4}}>Search for people above</div></div>:following.map(p=><div key={p.id} className="friend-card" onClick={()=>viewProfile(p.id)}><div className="friend-avatar">{p.avatar_url?<img src={p.avatar_url} alt=""/>:(p.name||"?")[0].toUpperCase()}</div><div style={{flex:1}}><div style={{fontWeight:700,fontSize:14}}>{p.name}</div><div style={{fontSize:12,color:"var(--gray-400)"}}>{p.bio}</div></div><button className="btn btn-secondary btn-sm" onClick={e=>{e.stopPropagation();toggleFollow(p.id);}}>Unfollow</button></div>)}</>}
  ...
  <footer className="footer"><div className="footer-inner">
    <div className="footer-brand"><div className="footer-brand-name"><img src="/logo.png" alt="" style={{height:28,width:"auto"}}/><span style={{color:"var(--pink-400)"}}>stew</span>dium</div><div className="footer-brand-desc">Your home for discovering, sharing, and organizing recipes.</div></div>
    <div className="footer-col"><h4>Explore</h4><button className="footer-link" onClick={()=>setPage("home")}>Browse Recipes</button><button className="footer-link" onClick={()=>{if(!user)setAuthModal("login");else setPage("planner");}}>Meal Planner</button><button className="footer-link" onClick={()=>{if(!user)setAuthModal("login");else setPage("friends");}}>Find Friends</button></div>
