...
export async function likeRecipe(userId, recipeId) {
  const { error } = await supabase
    .from('recipe_likes').insert({ user_id: userId, recipe_id: recipeId });
  return { error };
}

export async function unlikeRecipe(userId, recipeId) {
  const { error } = await supabase
    .from('recipe_likes').delete()
    .eq('user_id', userId).eq('recipe_id', recipeId);
  return { error };
}
...
export async function getUserLikes(userId) {
  const { data, error } = await supabase
    .from('recipe_likes').select('recipe_id').eq('user_id', userId);
  return { data: data?.map(d => d.recipe_id) || [], error };
}

export async function getRecipeLikeCount(recipeId) {
  const { count, error } = await supabase
    .from('recipe_likes').select('*', { count: 'exact', head: true })
    .eq('recipe_id', recipeId);
