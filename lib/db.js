...
export async function getRecipes({ category, search } = {}) {
  let query = sb().from('recipes').select('*, profiles!recipes_user_id_fkey(name, avatar_url)').order('created_at', { ascending: false });
...
export async function getRecipe(id) {
  const { data, error } = await sb().from('recipes').select('*, profiles!recipes_user_id_fkey(name, avatar_url)').eq('id', id).single();
  return { data, error };
}
...
export async function getUserRecipes(userId) {
  const { data, error } = await sb().from('recipes').select('*, profiles!recipes_user_id_fkey(name, avatar_url)').eq('user_id', userId).order('created_at', { ascending: false });
  return { data: data || [], error };
}
export async function getPublicRecipesByUser(userId) {
  const { data, error } = await sb().from('recipes').select('*, profiles!recipes_user_id_fkey(name, avatar_url)').eq('user_id', userId).eq('is_public', true).order('created_at', { ascending: false });
  return { data: data || [], error };
}

// ─── SAVED RECIPES ───
export async function getSavedRecipes(userId) {
  const { data, error } = await sb().from('saved_recipes').select('recipe_id, recipes(*, profiles!recipes_user_id_fkey(name, avatar_url))').eq('user_id', userId).order('created_at', { ascending: false });
...
export async function getCookedPhotos(recipeId) {
  const { data } = await sb().from('cooked_photos').select('*, profiles!cooked_photos_user_id_fkey(name)').eq('recipe_id', recipeId).order('created_at', { ascending: false });
  return { data: data || [] };
}
export async function addCookedPhoto(userId, recipeId, imageUrl) {
  const { data, error } = await sb().from('cooked_photos').insert({ user_id: userId, recipe_id: recipeId, image_url: imageUrl }).select('*, profiles!cooked_photos_user_id_fkey(name)').single();
  return { data, error };
}

// ─── COMMENTS ───
export async function getComments(recipeId) {
  const { data } = await sb().from('recipe_comments').select('*, profiles!recipe_comments_user_id_fkey(name, avatar_url)').eq('recipe_id', recipeId).order('created_at', { ascending: true });
  return { data: data || [] };
}
export async function addComment(userId, recipeId, text) {
  const { data, error } = await sb().from('recipe_comments').insert({ user_id: userId, recipe_id: recipeId, text }).select('*, profiles!recipe_comments_user_id_fkey(name, avatar_url)').single();
  return { data, error };
}
...
export async function getFollowing(userId) {
  const { data } = await sb().from('follows').select('following_id, profiles!follows_following_id_fkey(id, name, bio, avatar_url)').eq('follower_id', userId);
  return { data: data?.map(d => d.profiles).filter(Boolean) || [] };
}
export async function getFollowers(userId) {
  const { data } = await sb().from('follows').select('follower_id, profiles!follows_follower_id_fkey(id, name, bio, avatar_url)').eq('following_id', userId);
  return { data: data?.map(d => d.profiles).filter(Boolean) || [] };
}
...
// ─── NEWSLETTER ───
export async function subscribeNewsletter(email) { return await sb().from('newsletter_subscribers').upsert({ email }, { onConflict: 'email' }); }
